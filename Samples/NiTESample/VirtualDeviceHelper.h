...
{
public:
	typedef std::function<void(const openni::VideoFrameRef &,OniFrame *)>	TCallback;
...
public:
	...
	void onNewFrame( openni::VideoStream& rStream )
	{
		...
		if( rStream.readFrame( &mFrame ) == openni::STATUS_OK )
		{
			...
			{
				m_funcProccess( mFrame, pFrame );
                pFrame->height = mFrame.getHeight();
                pFrame->width = mFrame.getWidth();
