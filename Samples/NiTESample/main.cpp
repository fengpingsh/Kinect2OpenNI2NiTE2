...
int main( int argc, char** argv)
{
    ...
    if(argc < 2) {
        rc = virDevice.open("\\OpenNI2\\VirtualDevice\\Kinect");
        std::cout << "open virtual device(\\OpenNI2\\VirtualDevice\\Kinect ret = " << rc << std::endl;
    }else{
        rc = virDevice.open(argv[1]);
        std::cout << "open virtual device(" <<argv[1]<< ") ret = " << rc << std::endl;
    }
	openni::VideoStream* virDepth = CreateVirtualStream( virDevice, vsDepth, []( const openni::VideoFrameRef& mFrame,OniFrame* pFrame ){
        // type casting
        const openni::DepthPixel* pRealData = reinterpret_cast<const openni::DepthPixel*>( mFrame.getData() );
        openni::DepthPixel* pVirData = reinterpret_cast<openni::DepthPixel*>( pFrame->data );
    ...
    std::cout << "UserTracker successfully created" << std::endl;
